...
#[test]
fn send_sync_traits() {
    use super::super::{DirIter, FilterEntry};

    fn assert_send<T: Send>() {}
    fn assert_sync<T: Sync>() {}

    assert_send::<DirLister>();
    assert_sync::<DirLister>();
    assert_send::<DirIter>();
    assert_sync::<DirIter>();
    assert_send::<FilterEntry<DirIter, u8>>();
    assert_sync::<FilterEntry<DirIter, u8>>();
}
...
fn empty_file() {
    ...
    let wd = DirLister::new(dir.path().join("a"));
    let r = dir.run_recursive(wd);
    r.assert_no_errors();
...
fn sym_root_file_nofollow() {
    ...
    let wd = DirLister::new(dir.join("a-link"));
    let r = dir.run_recursive(wd);
    r.assert_no_errors();
...
fn skip_current_dir() {
    ...
    let mut paths = vec![];
    let mut it = DirLister::new(dir.path()).into_iter();
